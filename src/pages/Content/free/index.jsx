...
import { contents, getFreeContents } from "../../../store/content";
...
const FreeContent = () => {
  const freeDataContent = useSelector(contents);
  ...
  const { loadingFree, freeData, fetchFreeFailed } = freeDataContent;

  const dispatch = useDispatch();

  const load = (rows = 10, offset = 0) => {
    dispatch(getFreeContents({ limit: rows, offset, search }));
  };
  ...
  return (
    <div className="p-4">
      <div className="d-flex justify-content-between align-items-center py-4">
        <span className="page-title">Free Rounds</span>
      </div>
      <Table
        noSelect
        disableDownload
        ...
        actions={actions}
        pick={freeData?.model}
        tableName="Free Rounds"
        totalCounts={freeData?.totalCount || freeData?.model?.length}
        searchString={search}
        isSearchable
